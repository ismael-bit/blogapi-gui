...
function cargarComentarios(idPost) {
  ...
  //commentarios = '';
  blogapi
    .getPostCommets(idPost)
    .then(res => {
      res.forEach(p => {
        sp2 = commentsTemplate
          .replace("{{COMENTARIO}}", p.body)
      ...
      document.getElementById("app").innerHTML =
        postxxx + commentsTemplateH + sp + commentsTemplateF;
      document
        .getElementById("btnComentar")
        .addEventListener("click", Comentarevent);
    })
    .catch(err => console.error(err));
}
